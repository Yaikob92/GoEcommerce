...
(function() {
  ...
  /**
   * Wait for a global library to become available, then call callback.
   * Needed because afterInteractive scripts may load out of order.
   */
  function waitForLib(name, callback, interval, maxWait) {
  ...
  if (document.readyState === 'complete') { toggleScrolled(); } else { window.addEventListener('load', toggleScrolled); }
  ...
  if (preloader) {
    if (document.readyState === 'complete') { preloader.remove(); } else { window.addEventListener('load', () => { preloader.remove(); }); }
  }
  ...
  if (document.readyState === 'complete') { toggleScrollTop(); } else { window.addEventListener('load', toggleScrollTop); }
   ...
   */
  waitForLib('PureCounter', function() { new PureCounter(); });
   ...
   */
  waitForLib('bootstrap', initTooltips);
